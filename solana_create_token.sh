...
sudo solana-keygen grind --starts-with $Wallet_prefix:1
...
sudo spl-token initialize-metadata \
$filename \
$Token_name \
$Token_Symbol \
$Token_Uri

sudo spl-token create-account $filename

sudo spl-token mint $filename $Token_mintSupply
